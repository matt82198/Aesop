...
- **fleet/** — /fleet skill: one-shot fleet snapshot in JSON (agents, heartbeats, tracker lanes, orchestrator status) — see fleet/SKILL.md
- **buildsystem/** — /buildsystem skill: execute one complete orchestrated development wave (backlog rank → parallel Haiku fleet → merge train → checkpoint → audit) — see buildsystem/SKILL.md
