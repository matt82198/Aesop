...
2. **Opus capability verdict (2026-07-18 CLI run, 6/12 = 50%).** Opus showed 6 failures, split into: 3 reproducible semantic errors (file misclassification, wrong judgment), 2 formatting-wrapped answers with high variance (correct answer buried in explanation), 1 instability (passed rerun, failed original). Variant probe (explicit "bare answer only" instruction) **worsened** formatting verbosity, suggesting instruction-following degradation, not prompt-sensitivity. This anomaly is **unverified for the broader model** — scoped to this CLI config, system-prompt interaction, or instruction-following pattern; do NOT cite Opus 6/12 comparatively until re-run + inspected (tracked rc.5).
