...
- **mcp-fleet.test.mjs** — MCP fleet server test; verifies server startup and fleet status exports (run: `node --test tests/mcp-fleet.test.mjs`)
- **packaging-portability.test.mjs** — Package distribution test; verifies tarball contents, npm metadata (run: `node --test tests/packaging-portability.test.mjs`)
...
- **wizard.test.mjs** — Onboarding wizard integration test; verifies interactive CLI scaffolder flow (run: `node --test tests/wizard.test.mjs`)

## Python Test Suites (38 files)

**Serve/UI Module Tests** (14 files)
- **test_agent_detail_roundtrip.py** — Agent detail transcript rendering test; verifies multi-file agent log handling, parallel team support (run: `python tests/test_agent_detail_roundtrip.py`)
...
- **test_api_state.py** — State API endpoint tests; verifies snapshot aggregation, first-paint optimization (run: `python tests/test_api_state.py`)
...
- **test_ui_cost.py** — Cost view data and rendering test; verifies per-model/per-day aggregation, verdict scoring (run: `python tests/test_ui_cost.py`)
- **test_wave13_ui_correctness.py** — UI correctness regression suite; verifies a11y, contrast, keyboard navigation (run: `python tests/test_wave13_ui_correctness.py`)
...
**Tools/Scripts Module Tests** (17 files)
- **test_alert_bridge.py** — Alert webhook bridge test; verifies HTTP relay, signing, filtering (run: `python tests/test_alert_bridge.py`)
- **test_healthcheck.py** — Healthcheck skill liveness probe test; verifies signal collection integration (run: `python tests/test_healthcheck.py`)
- **test_metrics_gate.py** — Metrics verification gate test; validates self-stats drift detection (run: `python tests/test_metrics_gate.py`)
- **test_self_stats.py** — tools/self_stats.py verified metrics test; checks git log parsing, table formatting (run: `python tests/test_self_stats.py`)
- **test_state_store.py** — State store SQLite WAL test; verifies event append, projection rendering (run: `python tests/test_state_store.py`)
- **test_symlink_guard.py** — Path-traversal symlink/junction rejection test; verifies backup/UI handlers (run: `python tests/test_symlink_guard.py`)
